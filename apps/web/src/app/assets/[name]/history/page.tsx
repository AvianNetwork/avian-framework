...
export default async function AssetHistoryPage({
  ...
  const [historyResult, stats, asset] = await Promise.all([
    api.getAssetHistory(assetName, page).catch(() => ({ data: [] as Sale[], total: 0, page: 1, pageSize: 20 })),
    api.getMarketplaceStats(assetName).catch(() => null),
    api.getAsset(assetName).catch(() => null),
  ]);

  const sales = historyResult.data as Sale[];
    ...
    <div className="space-y-6">
      ...
      {sales.length === 0 ? (
        <div className="card text-center py-16 space-y-4">
          ...
          <p className="text-gray-300 font-semibold text-lg">No sales recorded yet</p>
          <p className="text-gray-500 text-sm">
            Completed trades for <span className="text-avian-400">{assetName}</span> will appear here.
          </p>
        </div>
      ) : (
                ...
                {sales.map((sale) => (
                  <tr key={sale.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-semibold text-avian-400 whitespace-nowrap">
                      {Number(sale.priceAvn)} <span className="text-gray-500 font-normal">AVN</span>
                    </td>
                    <td className="px-4 py-3 text-gray-300">{Number(sale.assetAmount)}</td>
                    <td className="px-4 py-3 text-gray-400 font-mono text-xs">{short(sale.sellerAddress)}</td>
                    <td className="px-4 py-3">
                      <ArrowRight className="w-3 h-3 text-gray-600" />
                    </td>
                    <td className="px-4 py-3 text-gray-400 font-mono text-xs">
                      {sale.buyerAddress ? short(sale.buyerAddress) : '—'}
                    </td>
                    <td className="px-4 py-3">
                      {sale.txid ? (
                        <span className="font-mono text-xs text-blue-400" title={sale.txid}>
                          {sale.txid.slice(0, 8)}…
                        </span>
                      ) : (
            ...
            {sales.map((sale) => (
              <div key={sale.id} className="p-4 space-y-2">
                <div className="flex justify-between">
                  <span className="text-xs text-gray-500">{formatDate(sale.soldAt)}</span>
                  <span className="font-semibold text-avian-400">
                    {Number(sale.priceAvn)} AVN
                  </span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-400">
                  <span className="font-mono">{short(sale.sellerAddress)}</span>
