...
export default function ReceivedBlindOffersPage() {
  ...
  async function handleAccept(id: string) {
    ...
    try {
      ...
      // Success — navigate buyer to their listing
      setOffers((prev) => prev.filter((o) => o.id !== id));
      setAcceptingId(null);
      setPsbtInput('');
      alert(
        `Offer accepted! The buyer can now complete the purchase.\n\nListing ID: ${result.listingId}\nOffer ID: ${result.offerId}`
      );
    } catch (e: unknown) {
              ...
              {acceptingId === offer.id ? (
                <div className="mt-4 pt-4 border-t border-gray-800 space-y-3">
                  <p className="text-sm text-gray-300">
                    <strong>Step 1</strong> — In Avian Core console, build a PSBT for{' '}
                    <code className="text-avian-400">{offer.assetName}</code> (amount:{' '}
                    {Number(offer.assetAmount).toLocaleString()}, price:{' '}
                    {Number(offer.offeredPriceAvn).toLocaleString()} AVN):
                  </p>
...
# 3. Sign with SINGLE|ANYONECANPAY
walletprocesspsbt "PSBT_FROM_STEP_2" true "SINGLE|ANYONECANPAY"
...
# Paste the "psbt" value from the result below`}
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      onClick={() => { setAcceptingId(null); setPsbtInput(''); setAcceptError(null); }}
                ...
                <div className="mt-3 pt-3 border-t border-gray-800 flex gap-3 justify-end">
                  <button
                    onClick={() => { setAcceptingId(offer.id); setAcceptError(null); }}
